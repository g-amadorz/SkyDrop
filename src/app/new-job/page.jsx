...
            // Leaflet icon is imported implicitly by dynamic-loaded Marker/Popup
...
              // Use L.divIcon for the small dot
...
  // Determine the names of the nearest stations for start/end APs, if they exist
  const startNearestStation = startAccessPoint ? startAccessPoint.nearestStation : null;
  const endNearestStation = endAccessPoint ? endAccessPoint.nearestStation : null;
...
          {openPath.length > 1 && (
            <>
              {/* Polyline for stations/APs */}
              <Polyline
                positions={openPath.map(node => {
                  // Use the AP coordinates if the node is an access point, otherwise use station coordinates
...
              {startAccessPoint && startNearestStation && stationCoords[startNearestStation] && (
                <Polyline
                  positions={[
                    [startAccessPoint.lat, startAccessPoint.lng],
                    stationCoords[startNearestStation]
                  ]}
                  pathOptions={{ color: 'gray', dashArray: '4', weight: 2 }}
                />
              )}
              {endAccessPoint && endNearestStation && stationCoords[endNearestStation] && (
                <Polyline
                  positions={[
                    [endAccessPoint.lat, endAccessPoint.lng],
                    stationCoords[endNearestStation]
                  ]}
                  pathOptions={{ color: 'gray', dashArray: '4', weight: 2 }}
                />
              )}

              {/* Markers - Only render dots for intermediate stations that are part of the train path.
                  Start and End markers (AP or Station) and the nearest-station-dots for APs are handled *above*
                  in the `markers` useMemo and in the first two conditional blocks in the openPath section. */}
              {openPath.slice(1, -1).map((node, idx) => {
                const pos = stationCoords[node];
                if (!pos) return null;

                // Do not render a dot if this intermediate station is the starting or ending nearest station,
                // as that nearest-station dot is already explicitly rendered above.
                if (node === startNearestStation || node === endNearestStation) {
                  return null;
                }
...
                });
                return <Marker key={node} position={pos} icon={icon} interactive={false} />;
              })}
            </>
          )}
        </MapContainer>
